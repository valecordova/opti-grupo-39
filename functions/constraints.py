...
def create_restrictions_dict(x_crh, y_clh, u_lh, v_rh, nc_ch, ex_c):
	...
	# R6: La carga del camion al final del dıa deber ser igual a la CMAXc
	r6 = (CMAX_c[c] == quicksum(y_clh[c, l, h] * VCL_cl[c, l] + x_crh[c, r, h] * VCR_cr[c, r] for h in Horas) 
			 for l in EstacionesLenta for r in EstacionesRapida for c in Camiones)

	# R7: Si una estaci ́on alcanza su desgaste maximo debe ser reparada
	# R7.1: Estacion lenta
	r7_1 = (quicksum(y_clh[c, l, h] * DL for h in range(1, h_special)) <= DLMAX + M * u_lh[l, h_special] 
				 for c in Camiones for l in EstacionesLenta)
	# R7.2: Estacion rapida
	r7_2 = (quicksum(x_crh[c, r, h] * DR for h in range(1, h_special)) <= DRMAX + M * v_rh[r, h_special] 
				 for c in Camiones for r in EstacionesRapida)
